...
routing switch with dynamic inputs, and Tag Join.

Clone directly into `ComfyUI/custom_nodes/` -- no symlinks or copying needed.

## DPE Global Seed (required controller)

Workflows must include exactly one **DPE Global Seed** node whenever any seeded
picker is present. Pickers have no `seed` inputs or outputs of their own.
...
| **CLIP Token Report** | `clip`, `text` (socket) | `report` preview | `report`, `overflow` |
...
# from inside the dynamic_prompt_engine repo folder
PYTHONPATH=".." python -m unittest discover -s tests -v
# frontend widget tests
cd web && node --test
```

AI coding agents: see [AGENTS.md](AGENTS.md) for architecture rules, seed-system
invariants, test conventions, and a new-node checklist.
